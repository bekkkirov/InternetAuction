...
namespace InternetAuction.BLL.Tests.Tests
{
    public class BidServiceTests
    {
        ...
        public async Task Add_ShouldThrowWithInvalidDate(decimal bidValue, int lotId)
        {
            ...
            var createModel = new BidCreateModel() { BidValue = bidValue };
        ...
        public async Task Add_ShouldThrowWhenBiddingOnYourOwnLot()
        {
            ...
            var createModel = new BidCreateModel() { BidValue = 999 };
            var bidderUserName = "user3";
            var lotId = 3;
        ...
        public async Task Add_ShouldAddWithCorrectData()
        {
            ...
            var createModel = new BidCreateModel() { BidValue = 200 };
            var bidderUserName = "user1";
            var lotId = 4;

            // Act
            await _biddingService.AddAsync(createModel, bidderUserName, lotId);

            // Assert
            _unitOfWorkMock.Verify(x => x.BidRepository.Add(It.Is<Bid>(b => b.BidValue == createModel.BidValue
                                                                                  && b.Bidder.UserName == bidderUserName)), Times.Once);
